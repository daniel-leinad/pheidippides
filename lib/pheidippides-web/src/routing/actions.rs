...
pub async fn authorize<T: AsyncRead + Unpin>(request: &mut Request<T>, app: Messenger<impl DataAccess, impl AuthService>) -> Response {
    ...
    let authorization_params: AuthorizationParams = form_data::from_str(&content).or_bad_request()?;

    let user_verification = app
        .verify_user(&authorization_params.login, authorization_params.password)
        .await.or_server_error()?;
...
pub async fn signup<T: AsyncRead + Unpin>(request: &mut Request<T>, app: Messenger<impl DataAccess, impl AuthService>) -> Response {
    let content = request.content().await.or_server_error()?;
    ...
    match app.create_user(&signup_params.login, signup_params.password).await.or_server_error()? {
        Some(user_id) => {
            let signup_response = serde_json::json!(SignupResponse{ success: true, errors: vec![] });
            let session_id = sessions::generate_session_id();
            let headers = vec![header_set_cookie(sessions::SESSION_ID_COOKIE, &session_id)];
            sessions::update_session_info(session_id, sessions::SessionInfo{ user_id }).or_server_error()?;

            Response::Json{ content: signup_response.to_string(), headers }
        },
        None => {
            let signup_response = SignupResponse{ success: false, errors: vec![SignupError::UsernameTaken] };

            Response::Json{content: serde_json::json!(signup_response).to_string(), headers: vec![]}
        },
    }
}

pub async fn send_message<D: DataAccess, A, T: AsyncRead + Unpin>(request: &mut Request<T>, app: Messenger<D, A>, receiver: &str) -> Response {
    ...
    app.send_message(params.message, user_id, receiver).await.or_server_error()?;

    Response::Html { content: "ok.".to_owned(), headers: Vec::new() }
}

pub async fn subscribe_new_messages<A, T: AsyncRead + Unpin>(request: &Request<T>, app: Messenger<impl DataAccess, A>, params: &str) -> Response {
    #[derive(Deserialize)]
    struct SubscribeNewMessagesParams {
        last_message_id: Option<String>,
    }

    let user_id = routing::get_authorization(request.headers()).or_server_error()?.or_bad_request()?;

    let subscribe_new_messages_params: SubscribeNewMessagesParams = form_data::from_str(params).or_bad_request()?;
    ...
    let last_event_id_header = request.headers().get(&CaseInsensitiveString::from("Last-Event-ID"));
    ...
    let subscription = app.subscribe_to_new_messages(user_id, starting_point).await.or_server_error()?;
    ...
    Response::EventSource { retry: None, stream }
}
